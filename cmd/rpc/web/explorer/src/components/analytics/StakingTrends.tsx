...
const StakingTrends: React.FC<StakingTrendsProps> = ({ fromBlock, toBlock, loading, validatorsData, blocksData, blockGroups }) => {
    ...
    const generateStakingData = () => {
        ...
        const baseReward = avgRewardPerValidator / 1000000 // Convert from micro to PROOF
            ...
            <div className="h-32 relative">
                ...
                {hoveredPoint && (
                    ...
                    >
                        ...
                        <div className="text-green-400">{formatNumber(hoveredPoint.value)} PROOF</div>
                    </div>
                )}

                {/* Y-axis labels */}
                <div className="absolute left-0 top-0 h-full flex flex-col justify-between text-xs text-gray-400">
                    <span>{formatNumber(maxValue)} PROOF</span>
                    <span>{formatNumber((maxValue + minValue) / 2)} PROOF</span>
                    <span>{formatNumber(minValue)} PROOF</span>
                </div>
            </div>
