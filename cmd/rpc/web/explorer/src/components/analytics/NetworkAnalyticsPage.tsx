...
const NetworkAnalyticsPage: React.FC = () => {
    ...
    useEffect(() => {
        if (cardData && supplyData && validatorsData && pendingData && paramsData) {
            ...
            setMetrics(prev => ({
                ...
                avgTransactionFee: sendFee / 1000000, // Convert from wei to PROOF
    ...
    const handleExportData = async () => {
        ...
        try {
            ...
            exportData.push(['Average Transaction Fee', metrics.avgTransactionFee.toFixed(6), 'PROOF', 'API (params.fee.sendFee)'])
            exportData.push(['Total Value Locked', metrics.totalValueLocked.toFixed(2), 'M PROOF', 'API (supply.staked)'])
            ...
            if (supplyData) {
                ...
                exportData.push(['Metric', 'Value', 'Unit', 'Source'])
                exportData.push(['Total Supply', supplyData.totalSupply || 0, 'PROOF', 'API'])
                exportData.push(['Staked Supply', supplyData.staked || supplyData.stakedSupply || 0, 'PROOF', 'API'])
                exportData.push(['Circulating Supply', supplyData.circulatingSupply || 0, 'PROOF', 'API'])
                exportData.push(['', '', '', ''])
            }
            ...
            if (paramsData?.fee) {
                ...
                exportData.push(['Send Fee', paramsData.fee.sendFee || 0, 'Micro PROOF', 'API'])
                exportData.push(['Stake Fee', paramsData.fee.stakeFee || 0, 'Micro PROOF', 'API'])
                exportData.push(['Edit Stake Fee', paramsData.fee.editStakeFee || 0, 'Micro PROOF', 'API'])
                exportData.push(['Unstake Fee', paramsData.fee.unstakeFee || 0, 'Micro PROOF', 'API'])
                exportData.push(['Governance Fee', paramsData.fee.governanceFee || 0, 'Micro PROOF', 'API'])
                exportData.push(['', '', '', ''])
            }
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-gray-400">
                            Comprehensive analytics and insights for the ProofArcade blockchain.
                        </p>
                    </div>
