...
const TransactionsPage: React.FC = () => {
    ...
    const normalizeTransactions = (payload: any): Transaction[] => {
        ...
        return transactionsList.map((tx: any) => {
            ...
            const rawType = tx.messageType || tx.type || tx.transaction?.type || 'send'
            const type = formatTxTypeLabel(rawType)
            const from = getExplorerFromAddress(tx)
            let to = getExplorerToAddress(tx)

            // For certificateResults, extract from reward recipients
            if ((String(rawType).toLowerCase() === 'certificateresults') && tx.transaction?.msg?.qc?.results?.rewardRecipients?.paymentPercents) {
                const recipients = tx.transaction.msg.qc.results.rewardRecipients.paymentPercents
                if (recipients.length > 0) {
                    to = recipients[0].address || 'N/A'
                }
            }
            const amount = getExplorerAmountMicro(tx)
    ...
    const filterConfigs: FilterProps[] = [
        ...
        },
        {
            ...
            step: 1,
            displayLabels: [
                { value: 0, label: '0 PROOF' },
                { value: 500, label: '500 PROOF' },
                { value: 1000, label: '1000+ PROOF' },
            ],
        },
        {
            type: 'search',
                        ...
                        <div className="relative pt-4">
                            <div className="relative flex items-center gap-3">
                                ...
                                <div className="px-2 py-1 bg-primary text-black text-xs font-medium rounded shadow-lg whitespace-nowrap">
                                    {(filterConfigs[3] as AmountRangeFilter).value >= 1000 ? "1000+" : (filterConfigs[3] as AmountRangeFilter).value} PROOF
                                </div>
                                <input
                                    type="number"
                                    min="0"
