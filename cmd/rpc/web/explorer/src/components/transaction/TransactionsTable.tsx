...
import { getTxTypeColor, getTxTypeIcon, is2048Tx } from '../../lib/transactionPresentation'
...
const TransactionsTable: React.FC<TransactionsTableProps> = ({
    ...
    const formatFee = (fee: number) => {
        if (!fee || fee === 0) return '0 PROOF'
        // Fee comes in micro denomination from endpoint, convert to PROOF
    ...
    const rows = sortedTransactions.map((transaction) => [
        ...
        // Type
        <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getTxTypeColor(transaction.type)}`}>
            <i className={`${getTxTypeIcon(transaction.type)} text-xs`} style={{ fontSize: '0.875rem' }}></i>
        ...
        <span className="text-white text-sm font-medium">
            {is2048Tx(transaction.type) ? (
                <span className="text-slate-300">Game Action</span>
